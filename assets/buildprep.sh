...
JQUERY_UI_VER_FN=$( echo "$JQUERY_UI_VER" | tr  '.' '_'  )
...
if [ ! -f jquery-ui-${JQUERY_UI_VER}.zip ]; then
    ...
    if  [[ "$OSTYPE" == "darwin"* ]]; then
        # !@#$% Apple has to do things differently than the rest of the world, so, for MacOS sed command...
        SED_ARGS="-i.DELETEME -e"
    fi
    sed ${SED_ARGS} "s%images/ui-icons_%chrome-extension://__MSG_@@extension_id__/assets/jquery-ui_1_13_2-images/ui-icons_%g" jquery-ui_${JQUERY_UI_VER_FN}.min.css
    sed ${SED_ARGS} "s%images/ui-icons_%chrome-extension://__MSG_@@extension_id__/assets/jquery-ui_1_13_2-images/ui-icons_%g" jquery-ui_${JQUERY_UI_VER_FN}.theme.min.css
fi
